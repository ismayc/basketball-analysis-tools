...
# Also arm the scroll reveals here rather than at the end of the body, so
# the copy is never painted visible and then yanked back to opacity 0.
THEME_BOOT = """<script>
(function () {
  try {
  ...
  try {
    if ("IntersectionObserver" in window &&
      ...
      // Failsafe: if the observer never runs (script error, blocked
      // inline JS), un-hide everything rather than serve a blank page.
      setTimeout(function () {
...
# Reveal each block as it scrolls in, staggering siblings so a grid of
# cards cascades instead of swinging in as one slab. Everything already on
# screen at load fires immediately, because the observer reports intersecting
# targets on first observation.
REVEAL_SCRIPT = """<script>
(function () {
  var root = document.documentElement;
  if (!root.classList.contains("reveal")) { return; }
  var els = document.querySelectorAll(
    "main > *:not(.hero):not(.tiles):not(.cards)," +
    " .hero > *, .tiles > *, .cards > *");
  var io = new IntersectionObserver(function (entries) {
    // Stagger within the batch that crossed together, not by absolute
    // sibling index: a card grid cascades, a lone paragraph scrolled to
    // on its own gets no delay at all.
    var n = 0;
...
/* scroll reveals — each block of the page rises a little as it enters
   the viewport, siblings staggered. The html.reveal gate is set in the
   head boot script, and only when the reader hasn't asked for reduced
   motion, so no-JS and reduce-motion readers get plain static copy. The
   grid containers are exempted: their cells animate individually. */
html.reveal main > *,
html.reveal .hero > *,
html.reveal .tiles > *,
html.reveal .cards > * {
...
html.reveal main > .hero,
html.reveal main > .tiles,
html.reveal main > .cards,
html.reveal .is-in { opacity: 1; transform: none; }
@media print {
  html.reveal main > *, html.reveal .hero > *,
  html.reveal .tiles > *, html.reveal .cards > * {
...
against independent external data before being believed, no analysis
number in any README is typed by hand (findings scripts
